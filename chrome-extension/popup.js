...
Стили написания:
- Креативный — яркий, эмоциональный язык. Метафоры и образные выражения. Яркие заголовки. Сильный призыв к действию. В long_description допустимы 3-5 эмодзи из разрешённого списка, но ТОЛЬКО внутри строки (после тире или в середине фразы). НИКОГДА не ставь эмодзи в начало строки.
...
VK:
- vk_universal: заголовок 3–40, текст (короткое описание) 3–90, длинное описание (long_description) МАКСИМУМ 450 символов с пробелами (жёсткий лимит — ни символа больше!), текст рядом с кнопкой (button_text) 3–30. Структура long_description (ОБЕ секции ОБЯЗАТЕЛЬНЫ): 1) вводная фраза (1-2 предложения), 2) «Мы предлагаем:» — 3-4 коротких пункта (выгоды: зарплата, график, бонусы, ДМС), 3) «Ваши задачи:» — 3-4 коротких пункта (обязанности). Баланс: ≈55% выгоды, ≈45% обязанности. Каждый пункт — одна строка, без длинных описаний. Используй одинарные переносы строк (\\n, не \\n\\n). НЕ ставь пробелы в конце строк. Для пунктов списка ВСЕГДА используй «— » (тире), НИКОГДА эмодзи в начале строки. Эмодзи допустимы только после тире или внутри текста (например: «— 📌 Оформление по ТК РФ»). Эмодзи: до 3 в long_description, 1 в text (в конце предложения), запрещены в headline и button_text.
